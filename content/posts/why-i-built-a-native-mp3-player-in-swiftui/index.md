...
> I just wanted to play my own MP3s on my iPhone in 2025. Turns out that’s weirdly hard—unless you pay Apple, use iTunes Match, or deal with clunky third-party apps. So I wrote my own app. [GitHub link](https://github.com/nexo-tech/music-app)

Nowadays we are deep into using subscription for various services, because software pricing models just opted to the Software as a service model. In many ways it may feel cool, but personally I found myself it got difficult to track every subscription I have — some are on Apple subscription settings (like iCloud or Apple Music), others are… somewhere (like Cursor subscription that I’ve recently purchased). Recently I wanted to reduce the number of subscriptions I pay for and found that I don’t really have a need for a music streaming service. Recommendations often are out of place and I feel that I personally don’t miss out much when I have a personally curated collection of music.

I had an Apple Music subscription and I saw that they have a an iCloud music sync — you put your music to your iTunes library and it gets synced across devices. Alas, shortly after cancelling my subscription I discovered that this functionality doesn’t work without Apple Music subscription, so I reduced my problem to the next thing — have my library of music stored in iCloud and stream the music from cloud so I could play it from multiple devices. Sure, Apple provides standalone service called iTunes Match ($24.99 a year), but it has downsides of replacing audio-files when songs are matched and you're tied to iTunes, which is quite archaic piece of software these days.
...
## My options (before I decided going full-developer mode)
...
### Full-text search with built-in SQLite library
...
#### **Two virtual indexes, two domains**
...
### Implementing audio player parts
...
## Summary and the final frustration
